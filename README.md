...
- Implemented detectors:
  ...
  - Dockerfile `ENV` / explicit `CMD` and `ENTRYPOINT` runtime args, docker-compose `environment`, and Kubernetes `ConfigMap.data` / Secret keys / container `env` / `envFrom` / config volumes / explicit `command` and `args` runtime definitions
