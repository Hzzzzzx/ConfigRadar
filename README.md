...
- Implemented detectors:
  ...
  - Dockerfile `ENV` / JVM option env / explicit `CMD` and `ENTRYPOINT` runtime args, docker-compose `environment` / explicit `command` and `entrypoint` runtime args, and Kubernetes `ConfigMap.data` / Secret keys / container `env` / `envFrom` / config volumes / explicit `command` and `args` runtime definitions
