...
final class RemoteConfigCenterCheckEnricherTest {
    @Test
    void addsWarningForRemoteSpringConfigImport() {
        var source = new SourceLocation("application.yml", 1, null, SourceKind.YAML, Scope.MAIN);
        var inventory = new ConfigInventory(
            null,
            ProjectInfo.unknown(),
            null,
            List.of(item("spring.config.import", "optional:nacos:demo", source)),
            List.of(),
            List.of(),
            List.of()
        );
