...
| 配置检测 | `ConfigDetector` | `SpringConfigFileDetector`、`DockerfileEnvDetector`、`JavaSourceConfigDetector` | Spring、Java、部署文件、配置中心、三方组件扫描器 |
...
- Dockerfile detector：扫描 `Dockerfile*` 中的 `ENV` 环境变量定义
