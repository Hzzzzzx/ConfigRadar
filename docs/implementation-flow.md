...
- CLI `diff`：读取两个 inventory，按 key-based 策略输出 `added/removed/changed/uncertainChanged/checks`；新增动态配置访问生成高风险 check，新增远程配置来源或敏感 key 名生成 warning check；支持 `--redact-sensitive` 在比较前遮盖敏感值
