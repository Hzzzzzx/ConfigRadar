...
- Runtime XML detector：扫描 `src/main/resources` 运行时 XML 中的 `${...}` 占位符；日志 XML 额外扫描 `<springProperty source="...">`
