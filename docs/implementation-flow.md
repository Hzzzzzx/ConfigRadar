...
- Deploy detectors：扫描 `Dockerfile*` 中的 `ENV` 与 `CMD` / `ENTRYPOINT` 显式 `--key=value`、`-Dkey=value` 运行参数、docker-compose `services.*.environment`、Kubernetes `ConfigMap.data`、Secret `stringData`/`data` key、container `env`、`envFrom`、config/secret volume 引用，以及 `command` / `args` 中显式 `--key=value`、`-Dkey=value` 运行参数；同批 YAML 可见的 `envFrom.configMapRef` 会展开为实际注入 key
