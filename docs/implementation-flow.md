...
- Deploy detectors：扫描 `Dockerfile*` 中的 `ENV`、docker-compose `services.*.environment`、Kubernetes `ConfigMap.data`、container `env` 与 `envFrom` 引用；同批 YAML 可见的 `envFrom.configMapRef` 会展开为实际注入 key
