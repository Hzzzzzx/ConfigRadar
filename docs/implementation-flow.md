...
- Deploy detectors：扫描 `Dockerfile*` 中的 `ENV`、JVM option env 内部 `-Dkey=value`、`CMD` / `ENTRYPOINT` 显式 `--key=value`、`-Dkey=value` 运行参数、docker-compose `services.*.environment`、JVM option env 与 `command` / `entrypoint` 显式运行参数、Kubernetes `ConfigMap.data`、Secret `stringData`/`data` key、container `env`、`envFrom`、config/secret volume 引用，以及 `command` / `args` 中显式 `--key=value`、`-Dkey=value` 运行参数；同批 YAML 可见的 `envFrom.configMapRef` 会展开为实际注入 key
...
- Generic config getter 覆盖：Typesafe `getString/getInt/getDuration/getStringList/hasPath/getConfig`，Apache Commons `getBoolean` 等 typed getter，MicroProfile `getValue/getOptionalValue/getConfigValue`
...
- CLI `diff`：读取两个 inventory，按 key-based 策略输出 `added/removed/changed/uncertainChanged/checks`；新增动态配置访问生成高风险 check，新增远程配置来源或敏感 key 名生成 warning check；支持 `--redact-sensitive` 遮盖 diff 输出中的敏感值，并保留敏感值发生变化的信号
